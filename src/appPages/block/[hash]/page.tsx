...
export default function BlockSinglePage({ params: { hash } }: { params: { hash: string } }) {
  ...
  const {
    ...
    error: blockError,
  } = useQuery(blockQK(BlockQueryKeys.block, hash), blockQueries.fetchBlock(hash), queryOptions);
  ...
  if (isBlockFetching || blockTransactionsResponse.isFetching)
    return <SkeletonPageWithTwoColumns />;

  if (!!blockError || blockTransactionsResponse.isError)
    return <AppBlockPageError error={blockError || blockTransactionsResponse.error} />;
