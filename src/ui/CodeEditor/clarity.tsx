export const clarity = (Prism: any) => {
  ...
  const language = {
    ...
    keyword: [
      {
        pattern: RegExp(
          `${par}(?:or|and|xor|not|begin|let|if|ok|err|unwrap\\!|unwrap-err\\!|unwrap-panic|unwrap-err-panic|match|try\\!|asserts\\!|\
...
define-read-only)${space}`
      ...
      {
        pattern: RegExp(
          `${par}(?:var-set|map-set|map-delete|map-insert|\
...
contract-call\\?)${space}`
      ...
      {
        pattern: RegExp(
          `${par}(?:list|map|filter|fold|len|concat|append|as-max-len\\?|to-int|to-uint|\
