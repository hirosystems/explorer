...
export function fetchRecentBtcBlocks(chain: string) {
...
export function fetchRecentStxBlocks(chain: string) {
  const apiUrl = getApiUrl(chain);
...
export async function fetchRecentBlocks(chain: string): Promise<RecentBlocks> {
  const [btcBlocksData, stxBlocksData] = await Promise.all([
    fetchRecentBtcBlocks(chain),
    fetchRecentStxBlocks(chain),
  ]);
