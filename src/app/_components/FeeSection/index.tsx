...
import StxThinIcon from '@/ui/icons/StxThinIcon';
import { Flex, Grid, HStack, Icon, Stack } from '@chakra-ui/react';
import { ArrowRight, Info, Lightning, PlusMinus } from '@phosphor-icons/react';
...
function FeeTabs() {
  ...
  const availableTxTypeFees = Object.keys(mempoolFeeResponse || {}).filter(
    key => key !== 'all'
  ) as MempoolFeeResponseKey[];
  const avgFee = mempoolFeeResponse?.all;
  return (
    <TabsRoot variant={'primary'} size={'redesignMd'} defaultValue={'average'} gap="4">
      <Stack gap={4} w="100%">
        <Stack gap={3}>
          <TabsList flexWrap={'wrap'} rowGap={2}>
            <TabsTrigger value="average" gap={1}>
              <TabIcon>
          ...
          <Flex>
            <TabsContent value="average">
              <HStack gap={2} flexWrap={'wrap'}>
                <FeeData title="Low" ustxValue={avgFee?.low_priority || 0} />
                <FeeData title="Standard" ustxValue={avgFee?.medium_priority || 0} />
                <FeeData
                  title={
                    <>
                      High{' '}
                  ...
                  ustxValue={avgFee?.high_priority || 0}
                />
              </HStack>
            </TabsContent>
            {availableTxTypeFees.map(txType => (
