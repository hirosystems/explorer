...
export async function GET(request: NextRequest) {
  ...
  console.log({ searchParams, blockBurnTime, LUNAR_CRUSH_API_KEY });
  ...
  if (!blockBurnTime || blockBurnTime === 'current') {
    ...
    console.log({ blockBurnTime, response });
  ...
  const url = `https://lunarcrush.com/api4/public/coins/stx/time-series/v2?bucket=day&interval=1d&start=${
    from.getTime() / 1000
  }&end=${to.getTime() / 1000}`;
  console.log({ url });
